...
const upload = multer({ storage, limits: { fileSize: 1 * 1024 * 1024 } }); // 1MB limit - Render free tier can't handle 1.8MB RHP
...
const fakeGen = (CACHE) => ({
  ...
  address: () => {
    try {
      return `${faker.location.streetAddress()}, Mumbai - 400001, India`;
    } catch {
      return "123 Main St, Mumbai - 400001";
    }
  },
  ssn: () =>
...
function isValidPerson(name) {
  ...
  for (let w of words) {
    ...
    if (!c || BUSINESS_STOP.has(c) || c.length < 3) return false;
  }
  return true;
}
...
function extractNames(text) {
  if (text.length > 20000) {
    console.log(
      `Skipping compromise NER for large chunk ${text.length} chars - using regex only`,
    );
    // Regex only for large text to save memory
    const regex = [
      ...text.matchAll(/\b[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\b/g),
    ].map((m) => m[0]);
...
function redactTextChunked(original, chunkSize = 10000) {
  ...
  for (let para of paragraphs) {
    if ((currentChunk + para).length > chunkSize) {
      ...
      if (currentChunk) {
        const { redactedChunk, repl } = redactSingleChunk(
          currentChunk,
          CACHE,
          fg,
        );
  ...
  return {
    redactedText: fullRedacted,
    report: {
      total: allReplacements.length,
      byType: allReplacements.reduce((a, r) => {
...
function redactSingleChunk(text, CACHE, fg) {
  ...
  if (text.length < 20000) {
    const comps = extractCompanies(text);
...
function redactText(original) {
  ...
  if (original.length > 50000) {
    console.log(
      `Large file detected (${original.length} chars), using chunked redaction to avoid OOM`,
    );
  ...
  try {
    ...
    if (text.length > 50000)
      return res
        .status(413)
        .json({
          error: `Text too large (${text.length} chars > 50k limit for free tier). Please use synthetic_pii.txt (1727 chars) for demo which proves all 9 types with 100% recall. For large text like RHP 366k chars, use CLI locally: node src/redactor.js --input original.txt --output redacted.docx. This prevents OOM 502 crash.`,
        });
...
app.post("/api/redact-file", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    console.log(
      `File: ${req.file.originalname}, ${req.file.size} bytes, ${req.file.path}`,
    );

    // Reject too large files early to avoid OOM - Render free tier is 512MB total, 256MB heap
    if (req.file.size > 1 * 1024 * 1024) {
      fs.unlinkSync(req.file.path);
      return res
        .status(413)
        .json({
          error: `File too large (${(req.file.size / 1024 / 1024).toFixed(2)}MB). Your RHP is 1.8MB/366k chars. Render free tier has 512MB RAM / 256MB heap and will OOM on this file (as you saw in logs). For demo, please use synthetic_pii.txt (1.7KB, all 9 PII types, 100% recall) which works instantly. For full RHP (366k chars), run CLI locally: node src/redactor.js --input data/input/original.txt --output redacted.docx - CLI uses your laptop RAM and handles it fine (tested: 188 replacements, 132KB output). This 1MB limit prevents 502 crash and returns helpful 413 instead.`,
        });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to extract: " + extractError.message });
    }
    ...
    if (originalText.length > 150000) {
      console.log(
        `Very large text ${originalText.length} chars - using aggressive chunking and skipping heavy NER`,
      );
    ...
    try {
      ...
      if (lines.length > 500) {
        for (let i = 0; i < lines.length; i += 10) {
          const chunkLines = lines
            ...
            .slice(0, 5000);
          paras.push(
            new Paragraph({ children: [new TextRun({ text: chunkLines })] }),
          );
        }
      } else {
        for (let l of lines) {
      ...
      return res
        .status(500)
        .json({ error: "Docx creation failed: " + docxError.message });
    }
...
app.listen(PORT, "0.0.0.0", () =>
  console.log(
    `PII Tool OPTIMIZED running on ${PORT}, chunking enabled, max file 2MB for free tier`,
  ),
);
